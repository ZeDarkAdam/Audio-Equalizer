...
config.setting = {
  ...
  "PRESETS": [
    ...
    {"name": "ZDA_Nora_Bas", "default": true, "gains": [5.0000, 4.0000, 3.0000, 2.1000, 1.1000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000]}
  ]
};

config.icon = (function () {
  let canvas = new OffscreenCanvas(2 * 19, 2 * 19);
  let context = canvas.getContext("2d", {"willReadFrequently": true});
  /*  */
	let size = function (e) {
    return Math.round((e / 4) * ((2 * 19) / 8));
  };
	/*  */
	return {
      ...
      app.tab.query.active(function (tab) {
        if (tab) {
          if (tab.url) {
            let values = [];
            let lines = [5, 15, 25];
            let blacklist = config.blacklist.domains;
            let whitelist = config.whitelist.domains;
            let hostname = (new URL(tab.url)).hostname;
            let status = config.addon.state === "OFF" ? "disable" : (blacklist.indexOf(hostname) !== -1 ? "error" : (whitelist.indexOf(hostname) !== -1 ? "disable" : "enable"));
            /*  */
            for (let i = 0; i < 3; i++) {
              let val = (parseFloat(eq[i * 3 + 1].gain, 10) + parseFloat(eq[i * 3 + 2].gain, 10) + parseFloat(eq[i * 3 + 3].gain, 10));
              val = (val !== 0) ? val / 3 : 0;
            ...
            /*  */
            app.storage.get({"selected": {"name": "Default"}}, async function (e) {
              let image = context.getImageData(0, 0, 2 * 19, 2 * 19);
              let title = "Audio Equalizer" + (status === "error" ? " :: Error" : (status === "disable" ? " :: Disabled" : " :: " + e.selected.name));
              /*  */
              app.button.title(tab.id, title);
              app.button.icon(tab.id, null, image);
            });
          }
        }
      });
